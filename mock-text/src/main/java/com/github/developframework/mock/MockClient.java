...
public class MockClient {
     ...
     * @return 生成的随机值
     */
    public String mock(InputStream inputStream, Charset charset) {
        StringBuffer templateBuffer = new StringBuffer();
        IOAdvice.forEach(inputStream, charset, line -> templateBuffer.append(line).append('\n'));
     ...
     * @return 生成的随机值
     */
    public List<String> mock(InputStream inputStream, Charset charset, int quantity) {
        StringBuffer templateBuffer = new StringBuffer();
        IOAdvice.forEach(inputStream, charset, line -> templateBuffer.append(line).append('\n'));
        List<String> list = new LinkedList<>();
        for (int i = 0; i < quantity; i++) {
